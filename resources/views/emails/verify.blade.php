...
<tr>
	<td style="padding:30px 0 50px 0">
		<div style="text-align:center">
			...
			{{{ trans('users.title_email_verify') }}}
			</div>
		<div style="font-size:22px;line-height:25px;letter-spacing:-1px;color:#888686">
		{{{ trans('users.click_below_email') }}}
	</div>
	<div style="margin:30px 0 35px 0">
		<a href="{{{ url('verify/account/' . $confirmation_code) }}}" style="display:inline-block;padding:4px 20px;font-weight:bold;line-height:24px;text-decoration:none;color:#f02d00;border:1px solid #f02d00;border-radius:3px" target="_blank">
			{{{ trans('users.confirm_my_account') }}}
			</a>
		</div>
</div>
